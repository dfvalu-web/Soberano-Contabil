...
export class CloudA3Signer {
  ...
  public signXmlDocument(request: CloudSignerRequest): Result<CloudSignerResponse, Error> {
    if (!request.conteudoXmlParaAssinar || typeof request.conteudoXmlParaAssinar !== 'string') {
      return Err(new Error('Conteñdo XML para assinatura em nuvem está vazio ou inválido.'));
    }
    ...
    const signatureNode = `<Signature xmlns="http://www.w3.org/2000/09/xmldsig#"><SignedInfo><CanonicalizationMethod Algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315"/><SignatureMethod Algorithm="http://www.w3.org/2001/04/xmldsig-more#rsa-sha256"/><Reference URI=""><DigestMethod Algorithm="http://www.w3.org/2001/04/xmlenc#sha256"/><DigestValue>${hash}</DigestValue></Reference></SignedInfo><SignatureValue>${signatureValue}</SignatureValue><KeyInfo><X509Data><X509Certificate>MIIF...[CERTIFICADO_ICP_BRASIL_A3_CLOUD]...</X509Certificate></X509Data></KeyInfo></Signature>`;

    const xmlAssinado = request.conteudoXmlParaAssinar.replace(/<\/([a-zA-Z0-9]+)>$/, `other signatureNode here`).replace('/<\/([a-zA-Z0-9]+)>$/', signatureNode + '</$1');
    ...
    return Ok({
      ...
      xmlAssinado: request.conteudoXmlParaAssinar + signatureNode,
      statusCertificado: 'VALIDO_ICP_BRASIL'
    });
  }
}
