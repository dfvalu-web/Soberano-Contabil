...
export interface VacationInput {
  salarioBase: number;
  diasGozoFerias: number; // 20 ou 30
  diasAbonoPecuniario: number; // 0 ou 10
...
export function calculateVacations(input: VacationInput): Result<VacationCalculationResult, Error> {
  ...
  const descontoInss = inssRes.success ? inssRes.data.valorInssTotal : 0;

  const baseTributavelIrrf = Number((baseTributavelInss - descontoInss - pensaoAlimenticia).toFixed(2));
  const irrfRes = calculateIrrf(baseTributavelIrrf, dependentesIrrf);
  const descontoIrrf = irrfRes.success ? irrfRes.data.valorIrrfDevido : 0;
...
export interface ThirteenthSalaryInput {
  salarioBase: number;
  mesesTrabalhadosNoAno: number; // 1 a 12
...
export function calculateThirteenthSalary(input: ThirteenthSalaryInput): Result<ThirteenthSalaryResult, Error> {
  ...
  if (parcela === 'PRIMEIRA') {
    ...
    const baseIrrf = Number((valorIntegral - descontoInss).toFixed(2));
    const irrfRes = calculateIrrf(baseIrrf, dependentesIrrf);
    const descontoIrrf = irrfRes.success ? irrfRes.data.valorIrrfDevido : 0;
...
const payrollIndex = `export * from './calculator/inss-irrf.js';
export * from './calculator/payroll-engine.js';
export * from './terminations/termination-engine.js';
export * from './benefits/vacation-thirteenth.js';
export * from './esocial/xml-generator.js';
`;
fs.writeFileSync('packages/core/src/payroll/index.ts', payrollIndex, 'utf8');
